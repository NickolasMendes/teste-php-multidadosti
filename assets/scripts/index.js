...
function DataClients() {
    ...
    const DisplayStyleClient = window.getComputedStyle(ClientTable).getPropertyValue('display');

    if (DisplayStyleClient === 'none') {
        $.ajax({
            url: "DataRequest.php",
            method: "GET",
            data: { action: "client" },
            success: function (data) {
                ...
                const createNewTbody = document.createElement('tbody');
                createNewTbody.innerHTML = 
                    `
                        ${parsedData.clientes.map(item => {
                            return (`
                            <tr>
                                <td> 
                                    ${item.nome}
                ...
                const tableResponsive = document.getElementById('ClientTable');
                
                tableResponsive.appendChild(createNewTbody);
            },
            error: function (xhr, status, error) {
                console.error("Erro na solicitação AJAX dos Clientes: " + error);
            }
        });
    }
}
...
function DataSuppliers() {
    ...
    const DisplayStyleSuppliers = window.getComputedStyle(SuppliersTable).getPropertyValue('display');

    if (DisplayStyleSuppliers === 'none') {
        $.ajax({
            url: "DataRequest.php",
            method: "GET",
            ...
            success: function (data) {
                ...
                createNewTbody.innerHTML = 
                    `
                        ${parsedData.fornecedores.map(item => {
                            return (`
                            <tr>
                                <td> 
                                    ${item.nome}
                ...
                const tableResponsive = document.getElementById('SuppliersTable');
                
                tableResponsive.appendChild(createNewTbody);
            },
            error: function (xhr, status, error) {
                console.error("Erro na solicitação AJAX dos Fornecedores: " + error);
            }
        });
    }
}
...
function DataUsers() {
    ...
    const DisplayStyleUsers = window.getComputedStyle(UsersTable).getPropertyValue('display');

    if (DisplayStyleUsers === 'none') {
        $.ajax({
            url: "DataRequest.php",
            method: "GET",
            data: { action: "user" },
            success: function (data) {
                ...
                const tableResponsive = document.getElementById('UsersTable'); //voltar aqui para apagar isso e usar o ja existente
                
                tableResponsive.appendChild(createNewTbody);
            },
            error: function (xhr, status, error) {
                console.error("Erro na solicitação AJAX dos Usuários: " + error);
            }
        });
    }
}
...
function ChangeSimpleTable(typeClickedBox) {
    SimpleTable.classList.add('changeColor');
    DefaultTable.style.display = 'none';
    
    switch (typeClickedBox) {
        case 'clients':
            SimpleTable.style.backgroundColor = '#27a9e3';
        ...
        case 'users':
            SimpleTable.style.backgroundColor = '#28b779';
            SuppliersTable.style.display = 'none';
        ...
        case 'suppliers':
            SimpleTable.style.backgroundColor = '#852b99';
            ClientTable.style.display = 'none';
            UsersTable.style.display = 'none';
